...
- `fgvm export` `[<...strings>]` [`-o|--output <path>`] [`-a|--archive`] [`-j|--json`] [`--manifest <path>`] prepares the matching editor and export templates, imports the project once, and exports every named preset in `export_presets.cfg` in declaration order.
    ...
    - `--archive` uses Godot's native `--export-pack` mode to produce one data-only ZIP pack per preset. It does not create a playable release archive; normal exports continue to use `--export-release`, and downstream delivery tools may package those playable outputs separately.
